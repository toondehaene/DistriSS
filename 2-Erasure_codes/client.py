...
def main():
    # Run with number of erasures (-e) 1 and 2 and again for delegated and not
    ...
    for file_id in file_ids:
        ...
        for i in range(args.measurements):
            ...
            # full_enc_times.append(results.get("full_dec_time"))
            # pure_enc_times.append(results.get("pure_enc_time"))

        df_client_get.insert(0, column=str(file_id), value=client_get_times) # file_id should be file
        # df_full_enc.insert(0, column=str(file_id), value=full_enc_times)
        # df_pure_enc.insert(0, column=str(file_id), value=pure_enc_times)
